...
export const PasskeysPanel = () => {
  ...
  const runDelete = async () => {
    ...
    const { error } = await authClient.passkey.deletePasskey({ id: deleteTarget.id });
    ...
    setPasskeys((prev) => prev?.filter((p) => p.id !== deleteTarget.id) ?? null);
      ...
      >
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="flex min-w-0 flex-col gap-0.5">
            <span className="truncate font-medium text-sm">{passkey.name || "Passkey"}</span>
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Your browser will prompt you to use Touch ID, Face ID, a security key, or your
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              You won&apos;t be able to sign in with this passkey anymore. You can add it again
              later.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
